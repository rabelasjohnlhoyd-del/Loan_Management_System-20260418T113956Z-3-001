    ...
    try:
        ...
        cursor.execute("""
            SELECT la.id, la.reference_no, u.full_name AS borrower_name,
                   lt.name AS type_name, la.amount_requested, la.status, la.submitted_at
...
def recommend_application(app_id):
    ...
    recommendation = request.form.get('recommendation')  # 'under_review' or 'flagged'
    ...
    try:
        ...
        log_activity('recommend_application',
                     f'App ID {app_id} set to {recommendation} by officer. Notes: {notes}')
...
def borrower_detail(borrower_id):
    """Officer tingnan ang full profile ng isang borrower."""
    try:
        conn = get_db()
        cursor = conn.cursor(dictionary=True)
...
def update_score(user_id):
    """Updates credit score and risk level ng borrower."""
...
def payment_list():
    ...
    try:
        ...
        query = """
            SELECT p.id, p.loan_id, p.amount_paid, p.payment_date,
                   p.payment_method, p.reference_no, p.status,
                   p.created_at,
                   l.loan_no, l.outstanding_balance,
                   u.full_name AS borrower_name, u.email AS borrower_email
            FROM payments p
        ...
        for p in payments:
            ...
            p['outstanding_balance'] = float(p.get('outstanding_balance') or 0)
    ...
    return render_template('O_payments.html',
                           payments=payments,
                           counts=counts,
                           status_filter=status_filter,
                           search=search)
...
def verify_payment(payment_id):
    ...
    try:
        ...
        if action == 'approved':
            ...
            cursor.execute("""
                UPDATE payments
                SET status = 'approved', verified_by = %s, verified_at = NOW(), notes = %s
                WHERE id = %s
            ...
            # Mark amortization period as paid
            cursor.execute("""
            ...
            # If fully paid, update borrower profile
            if new_balance <= 0:
                cursor.execute("""
                    UPDATE borrower_profiles
            ...
            log_activity('approve_payment',
                         f'Payment ID {payment_id} approved. Loan ID {payment["loan_id"]}. '
            ...
            cursor.execute("""
                UPDATE payments
                SET status = 'rejected', verified_by = %s, verified_at = NOW(), notes = %s
                WHERE id = %s
            """, (session.get('user_id'), notes, payment_id))
...
# ================================================================
# SECTION 9: API ENDPOINTS (para sa dashboard JS widgets)
# ================================================================
...
def api_stats():
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) AS cnt FROM users WHERE id_verification_status = 'pending'")
        pending_verifs = cursor.fetchone()['cnt']
